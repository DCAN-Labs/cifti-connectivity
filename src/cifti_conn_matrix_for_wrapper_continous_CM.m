function outfile = cifti_conn_matrix_for_wrapper_continous_CM(wb_command, ...
    ...
    make_dconn_conc, output_directory, dtseries_conc,use_continous_minutes,memory_limit_value)
...
% additional_mask = provide an additional file of 1s (keep) and 0s (remove) that is the same length as the dtseries, only use selected frames. % (e.g.you want to make a connectivity matrix of only the second half of the dtseries.)
        ...
        if strcmpi(minutes_limit, 'none')            
            ...
            elseif minutes_limit <= good_minutes
                if use_continous_minutes ==1
                   ...
                   minute_limit_frames = round(minutes_limit*60/TR);
