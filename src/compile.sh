...
/mnt/max/software/MATLAB/R2016b/bin/mcc -v -m -R -singleCompThread -R -nodisplay -o cifti_conn_template_for_wrapper cifti_conn_template_for_wrapper.m -a /mnt/max/home/conan/Documents/cifti_conn_wrapper/cifti_conn_matrix_exaversion.m -a /mnt/max/home/conan/Documents/cifti_conn_wrapper/isthisanoutlier.m
...
sed -i '/exe_dir=`dirname "$0"`/a if [ ! -d $TMPDIR/$USER ]; then\n    mkdir $TMPDIR/$USER\nfi\nexport MCR_CACHE_ROOT=$TMPDIR/$USER' run_cifti_conn_template_for_wrapper.sh
